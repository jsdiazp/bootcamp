...
if (
    ...
    $requirements = 'starship.exe', 'sd.exe'
    ...
    starship.exe preset gruvbox-rainbow -o $HOME\.config\starship.toml 
    ...
    if ($removeSystemIcon -eq "y") {
        ...
        sd.exe 'Windows = "󰍲"' 'Windows = ""' .\.config\starship.toml
    }

    sd.exe '"Documents" = "󰈙 "' '# "Documents" = "󰈙 "' .\.config\starship.toml
    sd.exe '"Music" = "󰝚 "' '"Music" = " "' .\.config\starship.toml
    sd.exe '"Developer" = "󰲋 "' '# "Developer" = "󰲋 "' .\.config\starship.toml
}

$setupVimrc = Read-Host "3. ¿Quieres activar una configuración básica para Vim? y/n"

if ($setupVimrc -eq "y") {
    $requirements = 'git.exe'
